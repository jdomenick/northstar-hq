...
export function FounderActivationStatus({ organizationId }: { organizationId: string }) {
  ...
  const profileComplete = Boolean(
    profile.data?.full_name && (profile.data?.title || profile.data?.bio),
  );
  ...
  return (
    ...
    >
      ...
      <ul>
        <StepRow
          label="Ventures"
          hint="Businesses SAM reasons across"
          count={vN}
          status={st(ventures, vN)}
          to="/labs/ventures"
          cta="Open"
        />
        <StepRow
          label="Projects"
          hint="Active work with objectives and status"
          count={pN}
          status={st(projects, pN)}
          to="/labs/projects"
          cta="Open"
        />
        <StepRow
          label="Goals"
          hint="Outcomes with a definition of success"
          count={gN}
          status={st(goals, gN)}
          to="/labs/goals"
          cta="Open"
        ...
        <StepRow
          label="Decisions"
          hint="Decision history and rationale"
          count={dN}
          status={st(decisions, dN)}
          to="/labs/decisions"
          cta="Open"
        />
        <StepRow
          label="Commitments"
          hint="Accountability items you owe or are owed"
          count={cN}
          status={st(commitments, cN)}
        ...
        <li className="flex items-center gap-4 border-b border-border/50 py-3">
          <span
            ...
            className={
              "h-1.5 w-1.5 shrink-0 rounded-full " +
              (profileComplete ? "bg-primary" : "bg-muted-foreground/25")
            }
          />
          <div className="min-w-0 flex-1">
            <div className="text-[13.5px] text-foreground">Founder profile</div>
            <div className="truncate text-[11.5px] text-muted-foreground">
              Name plus a title or bio so SAM knows who it works for
            </div>
          </div>
        ...
        <li className="flex items-center gap-4 py-3">
          <span
            ...
            className={
              "h-1.5 w-1.5 shrink-0 rounded-full " +
              (orgComplete ? "bg-primary" : "bg-muted-foreground/25")
            }
          />
          <div className="min-w-0 flex-1">
