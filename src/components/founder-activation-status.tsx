...
type StepStatus = "complete" | "empty" | "loading";
    ...
    <li className="flex items-center gap-4 border-b border-border/50 py-3 last:border-0">
      <span
        ...
        className={
          ...
          (status === "complete" ? "bg-primary" : status === "loading" ? "bg-muted-foreground/40" : "bg-muted-foreground/25")
        }
      />
      ...
      <div className="shrink-0 text-right">
        ...
        <div className="text-[10.5px] uppercase tracking-[0.18em] text-muted-foreground/70">
          {status === "complete" ? "on record" : status === "loading" ? "reading" : "none yet"}
        </div>
      </div>
      <Link
        to={to}
...
export function FounderActivationStatus({ organizationId }: { organizationId: string }) {
  ...
  const st = (q: { isLoading: boolean }, n: number): StepStatus =>
    q.isLoading ? "loading" : n > 0 ? "complete" : "empty";
    ...
    >
      ...
      <ul>
        ...
        <li className="flex items-center gap-4 border-b border-border/50 py-3">
          ...
          <Link to="/settings" className="shrink-0 rounded-md border border-border px-2.5 py-1 text-[11.5px] text-muted-foreground hover:bg-secondary/60 hover:text-foreground">Edit</Link>
        </li>
