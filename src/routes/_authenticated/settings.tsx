...
import { useAppearance, type FontSizePreference, type ThemePreference } from "@/lib/appearance";
  ...
  return (
    <div>
      <PageHeader
        eyebrow="Settings"
        title="Preferences"
        description="Tune how NorthStar Labs works for you and your team."
      />
      <PageBody>
          ...
          <div className="flex items-center justify-between gap-3">
            <div>
              ...
              <div className="text-muted-foreground">
                Seed NorthStar Labs with your real ventures, projects, goals, decisions, and
                commitments so SAM has the context it needs.
              </div>
            </div>
            <Link
              to="/settings/founder-activation"
              className="rounded-md border border-border px-3 py-1.5 text-[12.5px] hover:bg-secondary/60"
            >
              Open
            </Link>
          </div>
        ...
        <Tabs
          value={tab}
          onValueChange={(value) => navigate({ to: ".", hash: value, replace: true })}
        >
          <TabsList className="mb-10 -mx-2 h-auto flex-wrap justify-start gap-1 border-b border-border bg-transparent p-0">
            {SECTIONS.map((s) => (
              <TabsTrigger
                key={s.value}
                value={s.value}
                className="relative rounded-none border-b-2 border-transparent bg-transparent px-3 py-2.5 text-[12.5px] text-muted-foreground hover:text-foreground data-[state=active]:border-foreground data-[state=active]:bg-transparent data-[state=active]:text-foreground data-[state=active]:shadow-none"
              >
                {s.label}
              </TabsTrigger>
            ))}
...
function SamSettingsTab() {
  ...
  const Row = (props: { label: string; hint?: string; children: React.ReactNode }) => (
    <div className="grid grid-cols-[minmax(0,220px)_1fr] gap-6 border-b border-border/60 py-4 text-[13.5px] last:border-0">
      <div>
        ...
        {props.hint && (
          <div className="mt-0.5 text-[11.5px] text-muted-foreground/70">{props.hint}</div>
        )}
      </div>
  ...
  return (
    <div className="max-w-2xl">
      <Section
        title="SAM behavior"
        hint="How SAM answers, cites, and treats memory. Owner/admin only."
      >
        <Row label="Response style">
          <select
            disabled={disabled}
            value={s?.response_style ?? "balanced"}
            onChange={(e) => set({ response_style: e.target.value })}
            className="rounded-md bg-secondary/40 px-2 py-1.5 outline-none"
          >
        ...
        <Row label="Challenge level">
          <select
            disabled={disabled}
            value={s?.challenge_level ?? "balanced"}
            onChange={(e) => set({ challenge_level: e.target.value })}
            className="rounded-md bg-secondary/40 px-2 py-1.5 outline-none"
          >
        ...
        <Row label="Show citations" hint="Display source records under each SAM answer.">
          <SamToggle
            disabled={disabled}
            checked={s?.include_citations ?? true}
            onChange={(v) => set({ include_citations: v })}
          />
        </Row>
        <Row label="Show confidence" hint="Display SAM's confidence band and reasons.">
          <SamToggle
            disabled={disabled}
            checked={s?.show_confidence ?? true}
            onChange={(v) => set({ show_confidence: v })}
          />
        </Row>
      </Section>
      <Section title="Memory">
        <Row
          label="Allow memory proposals"
          hint="SAM extracts candidate memory from conversations. Always requires human confirmation."
        >
          <SamToggle
            disabled={disabled}
            checked={s?.allow_memory_proposals ?? true}
            onChange={(v) => set({ allow_memory_proposals: v })}
          />
        </Row>
        <Row label="Include founder memory">
          <SamToggle
            disabled={disabled}
            checked={s?.include_founder_memory ?? true}
            onChange={(v) => set({ include_founder_memory: v })}
          />
        </Row>
        <Row label="Include organization memory">
          <SamToggle
            disabled={disabled}
            checked={s?.include_org_memory ?? true}
            onChange={(v) => set({ include_org_memory: v })}
          />
        </Row>
        <Row label="Include venture memory">
          <SamToggle
            disabled={disabled}
            checked={s?.include_venture_memory ?? true}
            onChange={(v) => set({ include_venture_memory: v })}
          />
        </Row>
        <Row label="Memory review reminders" hint="Nudge to review expired or stale memory.">
          <SamToggle
            ...
            onChange={(v) => set({ memory_review_reminders: v })}
          />
        </Row>
      </Section>
      <Section title="Conversations">
        <Row label="Retain conversation history">
          <SamToggle
            disabled={disabled}
            checked={s?.retain_conversation_history ?? true}
            onChange={(v) => set({ retain_conversation_history: v })}
          />
        </Row>
      </Section>
    </div>
  ...
  async function handleRestore(row: {
    ...
    try {
      if (row.type === "project") await restoreProject.mutateAsync(row.id);
        ...
        await restoreKnowledge.mutateAsync({
          id: row.id,
          title: row.title,
          venture_id: row.ventureId ?? null,
        });
  ...
  return (
    <div className="space-y-6">
      <Section
        ...
        hint="Archived records stay in your organization and can be restored. Nothing is permanently deleted."
      >
          ...
          <ul className="divide-y divide-border/60">
            {(archivedQ.data ?? []).map((row) => (
              <li key={`${row.type}-${row.id}`} className="flex items-center gap-3 py-3">
                ...
                <button
                  onClick={() =>
                    navigate({ to: row.route.to as any, params: row.route.params as any })
                  }
        ...
        <p className="mt-4 text-[11.5px] text-muted-foreground/80">
          Restored records return with their previous relationships intact. Nothing is permanently
          deleted.
        </p>
      </Section>
    </div>
...
function ProfileTab() {
  ...
  const displayName = form.preferred_name?.trim() || form.full_name?.trim() || user?.email || "You";
  const initials =
    displayName
      .split(/\s+/)
      .filter(Boolean)
    ...
    <div className="max-w-2xl space-y-6">
      ...
      <Section title="Account">
        ...
        <p className="mt-3 text-[12px] text-muted-foreground">
          Your sign-in email and password are managed under the Security tab.
        </p>
      </Section>
...
function OrganizationTab() {
  ...
  async function save() {
    ...
    try {
      ...
      if (msg.toLowerCase().includes("duplicate") || msg.includes("organizations_slug"))
        toast.error("That slug is already taken.");
      else if (msg.toLowerCase().includes("invalid slug"))
        toast.error("Slug must use lowercase letters, digits, and hyphens.");
      ...
      <Section title="Organization">
        {!canManage && (
          <p className="mb-4 text-[12.5px] text-muted-foreground">
            Only owners and admins can change these fields.
          </p>
        )}
        <div className="grid grid-cols-2 gap-4 text-[13.5px]">
          <Fld label="Name">
        ...
        <div className="mt-4">
          <Fld label="Description">
            <textarea
              ...
              className="w-full resize-none bg-transparent outline-none disabled:opacity-70"
            />
          </Fld>
        </div>
      </Section>
  ...
  return (
    <div className="space-y-6">
      <Section title="Members">
        <p className="mb-4 text-[12.5px] text-muted-foreground">
          Internal member invites are not available: this workspace has no operator invite email
          flow wired. A person joins by signing up at the operator sign-in page, then appears here
          for a role and status assignment. Client-side invitations are separate and live on each
          client record.
        </p>
        {membersQ.isLoading ? (
            ...
            {members.map((m) => (
              <li key={m.id} className="flex items-center gap-4 py-4">
                ...
                <div className="flex items-center gap-2">
                  <span className="rounded-full bg-secondary/60 px-2 py-0.5 text-[11px] capitalize text-muted-foreground">
                    {m.status}
                  </span>
                  {canEditRoleOf(m) ? (
                    <select
                        ...
                        updateRole.mutate(
                          {
                            ...
                            role: next,
                            prevRole: m.role,
                            memberName: actorName(m.profile),
                          },
                          {
                            onError: (err) =>
                              toast.error(
                                err instanceof Error
                                  ? err.message.includes("final owner")
                    ...
                    <span className="rounded-md bg-secondary/40 px-2 py-1 text-[12px] capitalize text-muted-foreground">
                      {m.role}
                    </span>
                  )}
                  {canManage && m.status === "active" && canRemove(m) && (
                    <button
                      onClick={() => {
                        if (confirm(`Suspend ${actorName(m.profile)}?`))
                          updateStatus.mutate(
                            {
                              membershipId: m.id,
                  ...
                  {canManage && m.status !== "removed" && canRemove(m) && (
                    <button
                      onClick={() => {
                        if (confirm(`Remove ${actorName(m.profile)} from the organization?`))
                          updateStatus.mutate(
                            {
                              membershipId: m.id,
...
function Fld({ label, children }: { label: string; children: React.ReactNode }) {
  return (
    <label className="block rounded-lg bg-secondary/40 px-3 py-2.5">
      <div className="mb-1 text-[10.5px] uppercase tracking-[0.2em] text-muted-foreground/80">
        {label}
      </div>
      {children}
    </label>
...
function AppearanceTab() {
  ...
  return (
    <div className="max-w-3xl space-y-10">
      <Section
        title="Theme"
        hint="Applies across Command Center, Client Workspace, CRM, navigation, Settings, and SAM Messenger."
      >
          ...
          {THEME_OPTIONS.map((o) => (
            ...
            >
              <div className="mt-3 flex gap-1.5">
                <span
                  className={`h-6 flex-1 rounded-sm border border-border ${o.value === "light" ? "bg-white" : o.value === "dark" ? "bg-[oklch(0.11_0.008_255)]" : "bg-gradient-to-r from-white to-[oklch(0.11_0.008_255)]"}`}
                />
              </div>
            </OptionCard>
          ))}
      ...
      <Section
        title="Font size"
        hint="Scales interface typography without changing the density of the dashboards."
      >
        <div className="grid gap-3 sm:grid-cols-3">
          {FONT_OPTIONS.map((o) => (
            <OptionCard
...
function AccountabilityTab() {
  ...
  if (!activeOrgId) {
    return (
      <p className="text-[13.5px] text-muted-foreground">
        Select an organization to see accountability signals.
      </p>
    );
  }

  const loading =
    commitmentsQ.isLoading || projectsQ.isLoading || goalsQ.isLoading || decisionsQ.isLoading;
  ...
  return (
    <div className="max-w-4xl space-y-8">
      <Section
        ...
        action={
          <Link to="/labs/accountability" className="hover:text-foreground">
            Open accountability
          </Link>
        }
      >
        <div className="grid grid-cols-2 gap-3 sm:grid-cols-3 lg:grid-cols-5">
          ...
          <StatTile
            ...
            hint={`Next ${COMMITMENT_DUE_SOON_DAYS} days`}
          />
          <StatTile
            label="Stalled"
            value={stalled.length}
            hint={`No update in ${STALLED_PROJECT_DAYS} days`}
          />
          <StatTile
            label="Goals at risk"
            value={atRisk.length}
            hint={`Under ${GOAL_AT_RISK_PCT}% near target date`}
          />
      ...
      <Section
        title="Thresholds"
        hint="These rules drive every accountability signal across NorthStar Labs and SAM."
      >
        <ul className="text-[13px] text-muted-foreground">
          <li className="border-b border-border/50 py-2.5">
            A commitment is <span className="text-foreground">due soon</span> within{" "}
            {COMMITMENT_DUE_SOON_DAYS} days of its due date, and{" "}
            <span className="text-foreground">overdue</span> the day after.
          </li>
          <li className="border-b border-border/50 py-2.5">
            A project is <span className="text-foreground">stalled</span> after{" "}
            {STALLED_PROJECT_DAYS} days without an update while still open.
          </li>
          <li className="border-b border-border/50 py-2.5">
            A goal is <span className="text-foreground">at risk</span> inside {GOAL_AT_RISK_DAYS}{" "}
            days of its target date when progress is below {GOAL_AT_RISK_PCT}%.
          </li>
          <li className="py-2.5">
            A decision is <span className="text-foreground">awaiting you</span> when it is waiting
            on the founder, under your review, or past its review date.
          </li>
        </ul>
        <p className="mt-3 text-[12px] text-muted-foreground">
          Thresholds are fixed system rules today. Per-organization overrides are not stored
          anywhere yet, so nothing here is editable.
        </p>
      </Section>
    </div>
  );
  ...
  return (
    <div className="max-w-3xl space-y-8">
      <Section
        ...
        hint="Outbound email used for assessment alerts and client notifications."
      >
        {statusQ.isLoading ? (
            ...
            <div className="flex items-center gap-2 text-[13.5px]">
              <span
                className={
                  ...
                  (emailLive ? "bg-primary" : "bg-muted-foreground/40")
                }
              />
            ...
            {!emailLive && st && (
              <div className="rounded-md border border-border bg-card/40 p-3 text-[12.5px] text-muted-foreground">
                ...
                <ul className="mt-1 list-disc pl-5">
                  {st.missing.map((m) => (
                    <li key={m} className="font-mono text-[12px] text-foreground">
                      {m}
                    </li>
                  ))}
                </ul>
                <p className="mt-2">
                  Add these in Project Settings, Secrets. Nothing else is required in the app.
                </p>
              </div>
            )}
          </div>
      ...
      <Section
        ...
        hint="Stored on your organization's SAM settings. Owner and admin only."
      >
        {samQ.isLoading ? (
          <div className="h-16 animate-pulse rounded-md bg-card/30" />
        ) : (
          <div className="grid grid-cols-[minmax(0,320px)_1fr] items-center gap-6 border-b border-border/60 py-4 text-[13.5px] last:border-0">
            <div>
              ...
              <div className="mt-0.5 text-[11.5px] text-muted-foreground/70">
                Nudge you to review stale or expired SAM memory.
              </div>
            </div>
            <SamToggle
              disabled={!canManage}
        ...
        <p className="mt-3 text-[12px] text-muted-foreground">
          Per-user notification channels (digest email, push) are not backed by any table yet, so
          they are not shown as options.
        </p>
      </Section>
    </div>
  );
  ...
  return (
    <div className="max-w-2xl space-y-8">
      ...
      <Section
        ...
        hint="Supabase sends a confirmation link to the new address before the change takes effect."
      >
        <div className="grid gap-4 sm:grid-cols-[1fr_auto] sm:items-end">
          ...
          <button
            ...
            className="h-10 rounded-md border border-border px-3 text-[12.5px] text-foreground hover:bg-secondary/60 disabled:opacity-60"
          >
      ...
      <Section
        title="Sessions"
        hint="Signs you out of every browser and device, including this one."
      >
        <button
          onClick={signOutEverywhere}
          disabled={signingOut}
          className="rounded-md border border-border px-3 py-2 text-[12.5px] text-foreground hover:bg-secondary/60 disabled:opacity-60"
        >
          {signingOut ? "Signing out…" : "Sign out of all devices"}
        </button>
        <p className="mt-3 text-[12px] text-muted-foreground">
          Multi-factor authentication is not enabled on this project's auth configuration, so no MFA
          controls are shown.
        </p>
      </Section>
    </div>
