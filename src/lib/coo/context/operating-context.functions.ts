...
type Json = Database["public"]["Tables"]["organization_operating_context"]["Insert"]["active_ventures"];
...
export const upsertOrgOperatingContext = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }): Promise<OrgOperatingContextRow> => {
    ...
    const patch = stripUndefined({
      ...
      strategic_priorities: asJson(data.strategic_priorities),
      current_constraints: asJson(data.current_constraints),
      operating_principles: asJson(data.operating_principles),
      founder_preferences: asJson(data.founder_preferences),
      decision_preferences: asJson(data.decision_preferences),
      risk_tolerance: data.risk_tolerance ?? null,
      time_horizon: data.time_horizon ?? null,
      current_focus: data.current_focus ?? null,
      major_goals: asJson(data.major_goals),
      major_risks: asJson(data.major_risks),
      important_metrics: asJson(data.important_metrics),
      active_ventures: asJson(data.active_ventures),
      source_lineage: asJson(data.source_lineage),
...
export const upsertVentureOperatingContext = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }): Promise<VentureOperatingContextRow> => {
    ...
    const patch = stripUndefined({
      ...
      business_model: data.business_model ?? null,
      current_stage: data.current_stage ?? null,
      current_objectives: asJson(data.current_objectives),
      roadmap_summary: data.roadmap_summary ?? null,
      active_projects: asJson(data.active_projects),
      major_dependencies: asJson(data.major_dependencies),
      current_bottlenecks: asJson(data.current_bottlenecks),
      current_risks: asJson(data.current_risks),
      success_metrics: asJson(data.success_metrics),
      strategic_assumptions: asJson(data.strategic_assumptions),
      market_position: data.market_position ?? null,
      offers: asJson(data.offers),
      products: asJson(data.products),
      services: asJson(data.services),
      current_priorities: asJson(data.current_priorities),
      paused_priorities: asJson(data.paused_priorities),
